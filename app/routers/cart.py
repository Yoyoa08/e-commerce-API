...
def add_to_cart(item_in: schemas.CartItemAdd, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
...
def update_cart_item_quantity(item_id: int,
    ...
    if not cart_item:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Cart item with id {item_id} not found")
...
def remove_from_cart(item_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
